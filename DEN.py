...
class DEN(object):
    ...
    def build_model(self, task_id, prediction=False, splitting=False, expansion=None):
        ...
        if splitting:
            ...
            w = self.create_variable(
                'layer%d' % self.n_layers, 'weight_%d' % task_id, [prev_dim, self.n_classes], True)
            b = self.create_variable(
                'layer%d' % self.n_layers, 'biases_%d' % task_id, [self.n_classes], True)
            self.y = tf.matmul(bottom, w) + b
    ...
    def optimization(self, prev_W, selective=False, splitting=False, expansion=None):
        ...
        apply_grads = opt.apply_gradients(grads, global_step=self.g_step)  # update variables

        l1_var = [var for var in tf.trainable_variables()]
        l1_op_list = []  # in eq.2/3/5 ?
        ...
        GL_var = [var for var in tf.trainable_variables() if
                  'new' in var.name and ('bw' in var.name or 'tw' in var.name)]  # group lasso
        gl_op_list = []  # in eq.5
        with tf.control_dependencies([apply_grads]):
            for var in GL_var:
                ...
                th_t = self.gl_lambda
                gw = []
                for i in range(var.get_shape()[1]):  # groups by incoming weights
        ...
        if self.T == 1:
            ...
            for i in range(self.n_layers, 0, -1):
                if i == self.n_layers:
                    ...
                    selected_params['layer%d/weight_%d:0' % (i, task_id)] = \
                        w[np.ix_(all_indices['layer%d' % i], [0])]
                    ...
                    # T > 1 in here, self.load_param() in line 567
            ...
            for i in range(self.n_layers, 0, -1):
                if i == self.n_layers:
                    ...
                    params['layer%d/biases_%d:0' % (i, task_id)] = \
                    ...
                    temp_biases[all_indices['layer%d' % (i + 1)]] = \
            ...
            if c_loss < self.loss_thr:
                ...
                self.optimization(self.prev_W, expansion=True)  # update vars
                self.sess.run(tf.global_variables_initializer())

                print(' [*] Network expansion (training)')
                repeated, c_loss = self.run_epoch(
                    self.opt, self.loss, trainX, trainY, 'Train', print_pred=False)  # just calc loss
    ...
    def run_epoch(self, opt, loss, X, Y, desc='Train', selective=False, s_iter=0, print_pred=True):
        ...
        loss_window = collections.deque(maxlen=window_size)  # double-ended queue
